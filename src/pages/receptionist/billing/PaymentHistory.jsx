...
export default function PaymentHistory() {
  ...
  const getPaymentMethodIcon = (method) => {
    switch (method) {
      ...
      case 'card':
        return { icon: CreditCard, color: 'text-sage-600', bgColor: 'bg-blue-500/20' }
      case 'online':
        return { icon: Globe, color: 'text-purple-400', bgColor: 'bg-purple-500/20' }
      default:
        return { icon: DollarSign, color: 'text-sage-700', bgColor: 'bg-slate-500/20' }
    }
  }

  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-sage-50 via-sage-100 to-white text-sage-900 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-400 mx-auto mb-4"></div>
          <p className="text-sage-700">Loading payment history...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-sage-700">Track all payment transactions</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-sage-700 mt-2">${analytics.totalAmount.toLocaleString()}</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-3xl font-bold text-sage-600">{analytics.todayPayments}</p>
            <p className="text-sm text-sage-700 mt-2">${analytics.todayAmount.toLocaleString()}</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-sage-700 mt-2">${analytics.weekAmount.toLocaleString()}</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-sage-700 mt-2">${analytics.monthAmount.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-sage-700 mt-2">Total cash received</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-2xl font-bold text-sage-600">${analytics.cardPayments.toLocaleString()}</p>
            <p className="text-sm text-sage-700 mt-2">Total card payments</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-sage-700 mt-2">Total online payments</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-sage-700" />
                <input
                  type="text"
            ...
            {/* Method Filter */}
            <div>
              <label className="block text-sm font-medium text-sage-700 mb-2">Payment Method</label>
              <select
                value={methodFilter}
            ...
            {/* Date Filter */}
            <div>
              <label className="block text-sm font-medium text-sage-700 mb-2">Date Range</label>
              <select
                value={dateFilter}
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
          {filteredPayments.length === 0 ? (
            <div className="text-center py-12">
              <Receipt className="w-16 h-16 text-sage-700 mx-auto mb-4" />
              <p className="text-sage-700 text-lg">No payments found</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Payment ID</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Patient</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Invoice #</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Amount</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Method</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Reference</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Date</th>
                    <th className="text-left py-3 px-4 text-sage-700 font-medium">Actions</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={payment.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <div>
                            ...
                            <p className="text-sm text-sage-700">{payment.patientPhone}</p>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-sm text-sage-700">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex space-x-2">
                            <Link
                              ...
                              className="text-sage-600 hover:text-blue-300 p-1 hover:bg-blue-500/20 rounded transition-colors"
                              title="View Invoice"
                            >
