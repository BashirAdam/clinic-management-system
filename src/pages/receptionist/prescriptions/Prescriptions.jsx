        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-sage-700">View and manage patient prescriptions</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
        ...
        <div className="mb-8">
          ...
          {loading ? (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
              ...
              <p className="text-sage-700 mt-4">Loading prescriptions...</p>
            </div>
          ) : todayPrescriptions.length === 0 ? (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
              <FileText className="w-16 h-16 text-sage-700 mx-auto mb-4" />
              <p className="text-sage-700">No prescriptions for today</p>
            </div>
          ) : (
                ...
                <div key={prescription.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-sage-700">{prescription.patientAge || 'N/A'} years old, {prescription.patientGender || 'N/A'}</p>
                    </div>
                    <div className="flex space-x-2">
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sage-700">{prescription.prescriptionDate}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Users className="w-4 h-4 text-sage-700" />
                      <span className="text-sage-700">Dr. {prescription.doctorName}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Phone className="w-4 h-4 text-sage-700" />
                      <span className="text-sage-700">{prescription.patientPhone}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Mail className="w-4 h-4 text-sage-700" />
                      <span className="text-sage-700">{prescription.patientEmail}</span>
                    </div>
                  </div>
                  
                  {prescription.diagnosis && (
                    <div className="mb-4">
                      <h4 className="text-sm font-medium text-sage-700 mb-1">Diagnosis:</h4>
                      <p className="text-sm text-sage-700">{prescription.diagnosis}</p>
                    </div>
                  )}
                  
                  <div className="mb-4">
                    <h4 className="text-sm font-medium text-sage-700 mb-1">Medicines ({prescription.medicines?.length || 0}):</h4>
                    <div className="space-y-1">
                      {prescription.medicines?.slice(0, 3).map((medicine, index) => (
                        <p key={index} className="text-sm text-sage-700">
                          • {medicine.name} - {medicine.dosage}
                        </p>
                      ))}
                      {prescription.medicines?.length > 3 && (
                        <p className="text-sm text-sage-700">+{prescription.medicines.length - 3} more medicines</p>
                      )}
                    </div>
                  </div>
                      ...
                      <tr key={prescription.id} className="border-b border-white/5 hover:bg-white/5">
                        <td className="py-3 px-4">
                          <div>
                            ...
                            <p className="text-sm text-sage-700">{prescription.patientPhone}</p>
                          </div>
                        </td>
                        <td className="py-3 px-4 text-sage-700">{prescription.doctorName}</td>
                        <td className="py-3 px-4 text-sage-700">{prescription.prescriptionDate}</td>
                        <td className="py-3 px-4 text-sage-700">{prescription.diagnosis || 'N/A'}</td>
                        <td className="py-3 px-4 text-sage-700">{prescription.medicines?.length || 0} medicines</td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex space-x-2">
                            <Link
                              to={`/receptionist/prescriptions/view/${prescription.id}`}
                              className="px-2 py-1 bg-sage-500 hover:bg-sage-600 text-white rounded text-xs transition-colors"
                            >
                              View
                            </Link>
