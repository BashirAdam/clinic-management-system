...
export default function TokenQueue() {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'scheduled':
        return { color: 'text-sage-600 bg-blue-400/10', icon: ClockIcon }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sage-50 via-sage-100 to-white text-sage-900">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-sage-700">View and manage patient tokens for today</p>
            </div>
          </div>
          <LogoutButton />
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="w-20 h-20 bg-blue-500/30 rounded-2xl flex items-center justify-center">
                  <span className="text-4xl font-bold text-sage-600">{currentToken.tokenNumber}</span>
                </div>
                <div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
            <div>
              ...
              <p className="text-sage-700">{getTodayDisplay()}</p>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-sage-600">{queueStats.total}</div>
                <div className="text-sm text-sage-700">Total Tokens</div>
              </div>
              
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">{queueStats.waiting}</div>
                <div className="text-sm text-sage-700">Waiting</div>
              </div>
              
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-400">{queueStats.inProgress}</div>
                <div className="text-sm text-sage-700">In Progress</div>
              </div>
              
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{queueStats.completed}</div>
                <div className="text-sm text-sage-700">Completed</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-white border border-sage-200 rounded-lg text-sage-900 placeholder-sage-500 focus:border-sage-500 focus:ring-2 focus:ring-sage-200 focus:outline-none"
                />
              </div>
            </div>
        ...
        {/* Queue List */}
        <div className="bg-white/95 border border-sage-200 rounded-2xl shadow-lg shadow-sage-200/60 overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center min-h-96">
              <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-400 mx-auto mb-4"></div>
                <p className="text-sage-700">Loading queue...</p>
              </div>
            </div>
          ) : filteredAppointments.length === 0 ? (
            <div className="p-8 text-center">
              <div className="text-sage-700 text-lg mb-2">No patients in queue</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-white/5 border-b border-white/10">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Token</th>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Patient</th>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Contact</th>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Appointment</th>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Status</th>
                    <th className="px-6 py-4 text-left text-sm font-medium text-sage-700">Actions</th>
                  </tr>
                </thead>
                      ...
                      >
                          ...
                          {appointment.tokenNumber ? (
                            <div className="flex items-center space-x-2">
                              <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                                ...
                                <span className="text-xl font-bold text-sage-600">{appointment.tokenNumber}</span>
                              </div>
                              {isCurrentPatient && (
                            ...
                            <div className="text-sage-700">-</div>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            <div className="flex items-center space-x-2 text-sm">
                              <Phone className="w-3 h-3 text-sage-700" />
                              <span>{appointment.patientPhone}</span>
                            </div>
                            {appointment.patientEmail && (
                              <div className="flex items-center space-x-2 text-sm">
                                <Mail className="w-3 h-3 text-sage-700" />
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center space-x-2 text-sm">
                              <Clock className="w-3 h-3 text-sage-700" />
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <Link
                              ...
                              className="px-3 py-1 bg-sage-500 hover:bg-sage-600 text-white text-xs rounded-lg transition-colors"
